 ...
 */            (function(window, factory) {
    ...
    (() => {
        ...
        const slidesWrapper = document.querySelector(".projects__wrapper");
        document.querySelectorAll(".projects__slide");
        const filterButtons = document.querySelectorAll("[data-filter]");
        function filterSlides(currentFilter) {
            slidesWrapper.innerHTML = "";
            function doesBlockPassFilter(block) {
                const itemValues = block.dataset.filterItem.split(/\s*,\s*/);
                return itemValues.includes(currentFilter);
            }
            let currentSlide = document.createElement("div");
            currentSlide.className = "projects__slide swiper-slide";
            let blockCount = 0;
            filterItems.forEach((block => {
                if (doesBlockPassFilter(block)) {
                    if (blockCount === 4) {
                        slidesWrapper.appendChild(currentSlide);
                        currentSlide = document.createElement("div");
                        currentSlide.className = "projects__slide swiper-slide";
                        blockCount = 0;
                    }
                    const clonedBlock = block.cloneNode(true);
                    currentSlide.appendChild(clonedBlock);
                    blockCount++;
                }
            }));
            if (blockCount > 0) slidesWrapper.appendChild(currentSlide);
        }
        function handleFilterClick(e) {
            ...
            if (targetElement.hasAttribute("data-filter") && !targetElement.classList.contains("active")) {
                ...
                filterSlides(currentFilterValue);
            }
        }
